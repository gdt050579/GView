    ...
    : Window("Dissasm", "d:c,w:60%,h:60%", WindowFlags::ProcessReturn), buffer(_buffer), fa(_fa), size(_size)
{
    ...
    x64                      = Factory::RadioBox::Create(this, "x&64", "x:90%,y:3,w:10%,h:1", GROUPD_ID_ARCHITECTURE_TYPE, RADIOBOX_ID_ARCHITECTURE_X86, true);
    ...
    intel                      = Factory::RadioBox::Create(this, "&Intel", "x:90%,y:7,w:10%,h:1", GROUPD_ID_DESIGN_TYPE, RADIOBOX_ID_DESIGN_INTEL, true);
    ...
    little    = Factory::RadioBox::Create(this, "&Little", "x:90%,y:11,w:10%,h:1", GROUPD_ID_ENDIANESS_TYPE, RADIOBOX_ID_ENDIANESS_LITTLE, true);
...
bool DissasmDialog::Update()
{
    ...
    CHECK(dissasembler.Init(arm->IsChecked(), x64->IsChecked(), little->IsChecked()), false, "");
