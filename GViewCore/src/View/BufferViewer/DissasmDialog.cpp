...
bool DissasmDialog::Update()
{
    ...
    const auto size   = std::min<uint64>(instance->GetObject()->GetData().GetSize(), (uint64) 0x2000);
    const auto buffer = instance->GetObject()->GetData().Get(instance->GetCursorCurrentPosition(), (uint32) size, false);
