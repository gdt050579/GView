    ...
    : Window("Online analytics: results", "d:c,w:80,h:24", WindowFlags::None)
{
    ...
    Reference<Controls::TabPage> urlsTabPage         = Factory::TabPage::Create(this->tabs, "Urls", TAB_ID_URLS);
    ...
    for (Utils::Analysis& analysis : report->analysis) {
        capabilitiesListView->AddItem(
              { analysis.engine.c_str(), analysis.version.c_str(), analysis.result == Utils::AnalysisResult::Malicious ? "Malicious" : "Undetected" });
    }

    Reference<Controls::ListView> urlsListView = Factory::ListView::Create(urlsTabPage, "l:1,r:1,t:1,b:1", { "n:&Url,w:100%" });

    for (std::string& url : report->urls) {
        capabilitiesListView->AddItem(url.c_str());
    }
};
